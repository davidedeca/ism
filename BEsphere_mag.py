...
database = dir_path + '/data/BEsphereDB_mag.npy'
...
class BEsphere:
    ...
    def __init__(self, T, Pext, M, mu):
        ...
        self.y = np.interp(self.m, data_m[0:self.i_max], data_y[0:self.i_max])
        self.x = np.interp(self.m, data_m[0:self.i_max], data_x[0:self.i_max])

    def radius(self):
        rho0 = self.Pext / self.csound ** 2
        rhoc = rho0 / (1. - self.y)**3
        R = self.x / np.sqrt(G) / rhoc**(1./3.)
        return R
